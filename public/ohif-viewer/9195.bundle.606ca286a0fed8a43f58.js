...
const getMenuItemsDefault = ({
  ...
  const {
    servicesManager
  } = (0,_ohif_core__WEBPACK_IMPORTED_MODULE_2__/* .useSystem */ .Jg)();
  const {
...
function PanelStudyBrowser({
  ...
  const [expandedStudyInstanceUIDs, setExpandedStudyInstanceUIDs] = (0,react.useState)(studyMode === 'primary' && StudyInstanceUIDs.length > 0 ? [StudyInstanceUIDs[0]] : [...StudyInstanceUIDs]);
...
function Toolbar({
  ...
  const {
    ...
    closeItem,
    toggleLock
  } = (0,_ohif_core__WEBPACK_IMPORTED_MODULE_1__/* .useToolbar */ .tR)({
    buttonSection
  });
...
var lodash_uniqby = __webpack_require__(83424);
...
const callForAllDataSourcesAsync = async ({
  ...
  for (const dataSourceDef of dataSourceDefs) {
    ...
    if (!!configuration && dataSourceNames.includes(sourceName)) {
      ...
      const func = lodash_get_default()(dataSource, path);
  ...
  let results = [];
  if (mergeKey) {
    results = lodash_uniqby_default()(mergedData.flat(), obj => lodash_get_default()(obj, mergeKey));
  } else {
...
function getToolbarModule({
  ...
  // others
  {
    ...
    name: 'ohif.Toolbar',
    defaultComponent: Toolbar/* Toolbar */.M
  }, {
    name: 'evaluate.cine',
    evaluate: () => {
...
function init({
  ...
  toolbarService.registerEventForToolbarUpdate(hotkeysManager, [src/* HotkeysManager */.Lu.EVENTS.HOTKEY_PRESSED]);

  // Add
  src/* DicomMetadataStore */.H8.subscribe(src/* DicomMetadataStore */.H8.EVENTS.INSTANCES_ADDED, handleScalingModules);
  ...
  src/* DicomMetadataStore */.H8.subscribe(src/* DicomMetadataStore */.H8.EVENTS.SERIES_UPDATED, handleScalingModules);
