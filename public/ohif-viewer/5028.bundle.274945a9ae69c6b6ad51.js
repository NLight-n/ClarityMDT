"use strict";
(globalThis["webpackChunk"] = globalThis["webpackChunk"] || []).push([[5028,2591],{
...
function _getDisplaySetsFromSeries(instances, servicesManager, extensionManager) {
  ...
  const displaySet = {
    ...
    // by default for now
    displaySetInstanceUID: `pmap.${src/* utils */.Wp.guid()}`,
    SeriesDescription,
    SeriesNumber,
    SeriesDate,
    SOPInstanceUID,
    ...
    label: SeriesDescription || `${i18n_src/* default */.A.t('Series')} ${SeriesNumber} - ${i18n_src/* default */.A.t('PMAP')}`
  };
  ...
  displaySet.getReferencedVolumeId = () => {
    ...
    const referencedVolumeId = `${cornerstone_src.VOLUME_LOADER_SCHEME}:${referencedVolumeURI}`;
...
async function _load(displaySet, servicesManager, extensionManager, headers) {
  const volumeId = `${cornerstone_src.VOLUME_LOADER_SCHEME}:${displaySet.displaySetInstanceUID}`;
