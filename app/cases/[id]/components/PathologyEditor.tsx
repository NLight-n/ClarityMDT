...
export function PathologyEditor({
    ...
    const checkPermissions = async () => {
      ...
      try {
        const response = await fetch(
          `/api/cases/${caseId}/permissions?type=pathology`
        );
        if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/cases/${caseId}/pathology-findings`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pathologyFindings: processedContent }),
      });
      ...
      if (response.ok) {
        ...
        let errorMessage = "Failed to save pathology findings";
        try {
          const error = await response.json();
          errorMessage = error.error || errorMessage;
        } catch (e) {
          errorMessage = `Failed to save pathology findings (${response.status})`;
        }
        alert(errorMessage);
      }
    } catch (error) {
      console.error("Error saving pathology findings:", error);
